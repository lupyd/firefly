use firefly_core::rules::{get_username_from_signing_identity, does_signing_identity_has_this_username, on_signing_identity, has_permission};
use firefly_core::jwk::{generate_key, ecdsa_to_jwk};
...
use mls_rs::identity::{SigningIdentity, MlsCredential};
...
async fn test_signing_identity_helpers() {
    ...
    let signing_identity = SigningIdentity::new(credential.into_credential().unwrap(), signature_key);
    
    // Test get_username_from_signing_identity
    assert_eq!(get_username_from_signing_identity(&signing_identity).unwrap(), "alice");
    
    // Test does_signing_identity_has_this_username
    assert!(does_signing_identity_has_this_username("alice", &signing_identity));
    assert!(!does_signing_identity_has_this_username("bob", &signing_identity));
    
    // Test on_signing_identity
    let username = on_signing_identity(&signing_identity, |token| token.username.to_string()).unwrap();
