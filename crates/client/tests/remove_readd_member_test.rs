...
async fn setup_server() -> u16 {
    ...
    firefly_client::init_logger(format!("/tmp/firefly/test_remreadd_{}.log", port));
...
async fn test_remove_readd_member() {
    ...
    let alice_db = format!("/tmp/alice_remreadd_{}.db", test_run_id);
    ...
    let bob_db = format!("/tmp/bob_remreadd_{}.db", test_run_id);
