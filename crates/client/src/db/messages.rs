...
"#,
        ...
        let _ = conn
            ...
            .await;
        let _ = conn
            .execute("ALTER TABLE user_messages ADD COLUMN text TEXT NOT NULL DEFAULT ''")
            .await;

        // Backfill text column for any existing rows that lack extracted text before FTS triggers exist
        let unmigrated_rows = sqlx::query("SELECT rowid, message FROM user_messages WHERE text = '' AND length(message) > 0")
            .fetch_all(&mut *conn)
            .await?;
...
"#,
        ...
        let count_fts: i64 = sqlx::query_scalar("SELECT count(*) FROM user_messages_fts")
            .fetch_one(&mut *conn)
            .await
            .unwrap_or(0);
        let count_msgs: i64 = sqlx::query_scalar("SELECT count(*) FROM user_messages WHERE text != ''")
            .fetch_one(&mut *conn)
            .await
            .unwrap_or(0);
        if had_unmigrated || (count_fts == 0 && count_msgs > 0) {
            let _ = conn.execute("INSERT INTO user_messages_fts(user_messages_fts) VALUES('rebuild')").await;
        }
